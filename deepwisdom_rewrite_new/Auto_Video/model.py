...
class Model(LogicModel):
    ...
    def build(self):
        ...
        for key,model in enumerate(model_space):
            if model in [MC3]:
                # add hyperparam config
                Network = model(in_channels, num_classes=num_class, hyper_params=self.base_hyper_params)
    ...
    def init_opt(self):
        ...
        warmup_epoch = self.model.hyper_params['optimizer']['warm_up_epoch']
    ...
    def prediction(self, dataloader, model, checkpoints,test_time_augmentation=True, detach=True, num_step=None):
        ...
        with torch.no_grad():
            ...
            if self.USE_DEEPBLUE_ENSEMBLE:
                # # ----------------------------------result ensemble --------------------------------------
                result = {
                    'epoch': model.info['loop']['epoch'],
                    'pred': predictions,
                    # 'AUC': score
                }
                ...
                if len(self.results) > 4: 
                    index = np.argsort(np.array([v['valid']['score'] for v in checkpoints])) #
                    index = sorted(index[::-1][:model.hyper_params['checkpoints']['keep']]) #
                    list = [checkpoints[i]['epoch'] for i in index] #
                    epoches = [i for i in list if i in epoch_list]
                    predictions = np.array([v['pred'] for v in self.results if v['epoch'] in epoches][-5:])
