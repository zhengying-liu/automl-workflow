...
class LogicModel(Model):
    def __init__(self, metadata, session=None):
        ...
        ### 配置不同测试组合
        self.USE_DEEPBLUE_ENSEMBLE = True
        self.USE_FREIBURG_PARAMS = False
        ...
        ### 导入预训练模型
            ...
            num_classes = self.base_info['dataset']['num_class']
            ...
            # config_path = 'AutoDL_sample_code_submission/configs/deepwisdom.yaml'
            with open(config_path) as in_stream:
                model_config = yaml.safe_load(in_stream)

        # Freiburg's method of param config
        self.base_hyper_params = model_config['autocv']
        skip_valid_after_test = min(10, max(3, int(self.base_info['dataset']['size'] // 1000)))
        self.base_hyper_params["conditions"]["skip_valid_after_test"] = skip_valid_after_test
        ...
        ### Add ensembling-need variables from DeepBlueAI
        self.results = []
    ...
    def test(self, dataset, remaining_time_budget=None):
        if (self.ensembleconfig.MODEL_INDEX == 0) and (self.model == self.model_space[self.ensembleconfig.MODEL_INDEX]):
            print("Add base test ensemble from DeepBlueAI")
