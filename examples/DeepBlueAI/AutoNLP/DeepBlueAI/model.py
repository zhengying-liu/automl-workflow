...
#                 model = FT_tf_model(config)
                ...
                if self.cand_models[self.model_id] == 'CNN':
                    model = CNN_Model(self.data_id, self.seq_len, num_classes=self.metadata['class_num'], num_features=self.num_features, embedding_matrix=self.embedding_matrix)
                    ...
                    model = RNN_Model(self.data_id, self.seq_len, num_classes=self.metadata['class_num'], num_features=self.num_features, embedding_matrix=self.embedding_matrix)
...
#         return np.mean(self.best_test_res[1:feat_size], axis=0)
    ...
    def ensemble(self):
#         feat_size = len(self.his_scos) + 1
#         val_arr = np.array(self.best_val_res[1:feat_size])
#         test_arr = np.array(self.best_test_res[1:feat_size])

#         emodel = linear_stack(feature_size=feat_size-1, iters=10, rate=0.026, verbose=1)
#         #emodel.fit(val_arr, self.te_y)
        
        print ('ensemble scos:', self.his_scos)
        
        feat_size = len(self.his_scos) + 1
        val_arr = np.array(self.best_val_res)
        test_arr = np.array(self.best_test_res)
        print ('val_arr 0:', val_arr[0])
        print ('val_arr 1:', val_arr[1])
        print ('val_arr 2:', val_arr[2])
        emodel = linear_stack(feature_size=feat_size, iters=10, rate=0.026, verbose=1)

        #N = 10000
        #emodel.fit(val_arr[:,:N,:], self.te_y[:N])
        
        emodel.fit(val_arr, self.te_y)
        self.best_sco = emodel.pre_sco
        
        return emodel.predict(test_arr)
    ...
    def get_embedding(self, num_features, word_index, version='official'):
        ...
        if version == 'RENMIN':
            ...
            fasttext_embeddings_index = self.GET_EMBEDDING.fasttext_embeddings_index_zh if self.metadata['language'] == 'ZH' else self.GET_EMBEDDING.fasttext_embeddings_index_en
