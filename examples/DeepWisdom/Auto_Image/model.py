...
import skeleton
from architectures.resnet import ResNet9, ResNet18, BCEFocalLoss, FocalLoss
from skeleton.projects import LogicModel, get_logger
from skeleton.projects.others import NBAC, AUC, five_crop
...
class Model(LogicModel):
    ...
    def update_model(self):
        ...
        if self.is_multiclass():
            # self.model.loss_fn = BCEFocalLoss()
            ...
            # self.model.loss_fn = FocalLoss(num_class)
        ...
        # if self.info['dataset']['sample']['example']['is_255']:
        #     self.model_pred.reset_255()
        #     self.model.reset_255()
        self.init_opt()
        ...
        if self.hyper_params['conditions']['use_fast_auto_aug'] and \
            ...
            if len(searched_policy) > 0:
                ...
                self.dataloaders['train'].dataset.transform.transforms += [
                    lambda t: t.cpu().float() if isinstance(t, torch.Tensor) else torch.Tensor(t),
                    tv.transforms.ToPILImage(),
                    # tv.transforms.RandomResizedCrop(size=(int(self.input_shape[1]), int(self.input_shape[2]))),
                    skeleton.data.augmentations.Augmentation(
                        policy
                    ),
                    tv.transforms.ToTensor(),
    ...
    def epoch_valid(self, epoch, valid, reduction='avg'):
        ...
        with torch.no_grad():
            for step, (examples, labels) in enumerate(valid):
                ...
                LOGGER.debug(
                    '[valid] [%02d] [%03d/%03d] loss:%.6f AUC:%.3f, lr:%.8f',
                    epoch, step, num_steps, loss, auc,
        ...
        with torch.no_grad():
            ...
            for step, (examples, labels) in zip(range(num_step), dataloader):
                ...
                if self.use_test_time_augmentation and test_time_augmentation:
                    ...
                    # examples = five_crop(examples, (height, width))
                    # logits = model(examples, tau=tau)
                    # logits1, logits2, logits3, logits4, logits5 = torch.split(logits, batch_size, dim=0)
                    # logits = (logits1 + logits2 + logits3 + logits4 + logits5) / 5.0
                else:
                    logits = model(examples, tau=tau)
