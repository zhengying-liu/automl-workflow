...
class LogicModel(Model):
    def __init__(self, metadata, session=None):
        ...
        LOGGER.info('cat ' + self.metadata.get_dataset_name() + '/../*/* >/dev/null')
        os.system('cat ' + self.metadata.get_dataset_name() + '/../*/* >/dev/null')
    ...
    def build_or_get_train_dataloader(self, dataset):
        ...
        if aspect_ratio > 2 or 1. / aspect_ratio > 2:
            ...
            # LOGGER.info('Using ResNet18 for pedestrain')
            # self.model = self.model_18
            # self.model_pred = self.model_18_pred
        ...
        LOGGER.info('[input_shape] origin:%s aspect_ratio:%f target:%s', [times, height, width, channels], aspect_ratio,
                    input_shape)
        ...
        if num_class > batch_size / 2:
            ...
            # self.hyper_params['dataset']['steps_per_epoch'] = step_num // 2
        ...
        if must_shuffle or num_images < enough_count:
            ...
            LOGGER.info('[dataset] shuffle after split train/valid')
    ...
    def build_or_get_dataloader(self, mode, dataset=None, num_items=0):
        ...
        if mode == 'train':
            ...
            size_before = (int(self.input_shape[1] * 0.8), int(self.input_shape[2] * 0.8), 3)
            size_after = (self.input_shape[1], self.input_shape[2])
            # dataset = dataset.map(
            #     lambda *x: (preprocessor(tf.image.resize(tf.image.random_crop(x[0], size_before), size_after,
            #                                              method=tf.image.ResizeMethod.BICUBIC)), x[1]),
            ...
            if max(self.input_shape[1], self.input_shape[2]) >= 128:
                transform = tv.transforms.Compose([
                    # skeleton.data.Crop(int(self.input_shape[1] * 0.75), int(self.input_shape[2] * 0.75)),
                    # tv.transforms.Resize(int(self.input_shape[1]), int(self.input_shape[2]))
                ])
            else:
                transform = tv.transforms.Compose([
                    # skeleton.data.Crop(int(self.input_shape[1] * 0.8), int(self.input_shape[2] * 0.8)),
                    # tv.transforms.Resize(int(self.input_shape[1]), int(self.input_shape[2]))
                ])
    ...
    def train(self, dataset, remaining_time_budget=None):
        ...
        while True:
            ...
            LOGGER.info(
                ...
                metrics['train']['loss'], metrics['valid']['loss'], metrics['train']['score'],
                metrics['valid']['score'],
    ...
    def test(self, dataset, remaining_time_budget=None):
        ...
        # if self.last_predict is not None:
        #     rv = (rv + self.last_predict) / 2.
        # if self.update_transforms:
        #     self.last_predict = rv
