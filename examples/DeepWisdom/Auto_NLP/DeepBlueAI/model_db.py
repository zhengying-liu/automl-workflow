...
# print("Note: NLP DB deal_data, cwd={}".format(here))
# ac = cdll.LoadLibrary(os.path.join(here, 'ac.cpython-36m-x86_64-linux-gnu.so'))
...
class Model(object):
    ...
    def train(self, train_dataset, eval_dataset, remaining_time_budget=None):
        ...
        if self.START:
            ...
            x_eval,word_index, num_features, max_length  = self.deal_data(x_eval, data_lan, data_type=0, deal_seg=deal_seg, sentence_len=sentence_len)
            ...
            if self.epoch == 1 and self.model_id == 0:
                ...
                if self.data_id < 2:
                    ...
                    x_train = np.array(x_train,dtype='object')
                    x_train, word_index, num_features, max_length = self.deal_data(x_train,data_lan,data_type,deal_seg,sentence_len)
                    print("##### max_length is {} after deal_data".format(max_length))
                    x_eval = np.array(x_eval, dtype='object')
                    x_eval, word_index, num_features, max_length  = self.deal_data(x_eval, data_lan, data_type=0, deal_seg=deal_seg, sentence_len=sentence_len)
                    num_classes = self.metadata['class_num']
                    self.word_index = word_index
                ...
                if self.data_id == 1:
                    ...
                    self.embedding_matrix = self.get_embedding_new(num_features=self.num_features, word_index=self.word_index, fasttext_embeddings_index=self.fasttext_emb)
