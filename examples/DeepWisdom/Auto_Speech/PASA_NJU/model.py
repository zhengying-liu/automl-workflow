...
class Model(object):
    ...
    def train(self, train_dataset, remaining_time_budget=None):
        ...
        if self.train_loop_num == 1:
            ...
            self.train_dataset_num = len(train_dataset["x"])

        # zyadd: check if train_dataset update.
        if len(train_dataset["x"]) > self.train_dataset_num:
            self.model_manager.m_data_manager.update_train_dataset(train_dataset)
    ...
    def test(self, test_x, remaining_time_budget=None):
        ...
        log("pred_y from speech_pasa_model.py shape={}".format(pred_y.shape))
