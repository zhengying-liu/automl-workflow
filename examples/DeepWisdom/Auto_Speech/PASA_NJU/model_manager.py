...
from data_process import FEET_MODE, lr_preprocess_update

PREDICT_BBLEDING_START_LOOP_NUM = 6

# Model Hyper Params
LR_HP_MAXITER = 50


from eval_auc import autodl_auc as auc_metric


# def auc_metric(solution, prediction):
#     # fixme: 此处auc强制要求solution覆盖所有labels.
#     if solution.sum(axis=0).min() == 0:
#         log("Error: solution min 0")
#         return np.nan
#     auc = roc_auc_score(solution, prediction, average='macro')
#     log("auc_metric auc={}".format(auc))
...
class ModelManager(Classifier):
    ...
    def _pre_select_model(self, train_loop_num):
        ...
        if self._not_rise_num == self._patience \
            ...
            if self._model_idx == len(
                ...
                if not self.m_data_manager.crnn_first:
    ...
    def _blending_ensemble(self):
        ...
        log("blending ensemble selected len={}".format(len(selected)))
    ...
    def fit(self, train_loop_num=1, **kwargs):
        ...
        if self._round_num == 0:
            train_x, train_y, val_x, val_y = self.m_data_manager.get_train_data(train_loop_num=train_loop_num,
                                                                                model_num=self._model_num,
                                                                                round_num=self._round_num,
                                                                                use_new_train=self._use_new_train,
                                                                                use_mfcc=self._use_mfcc)
            self._is_nedd_30s = self.m_data_manager.need_30s
    ...
    def predict(self, test_x, is_final_test_x=False):
        self.predict_loop_num += 1

        x_val, y_val = self._val_set

        # fixme: show y_val y_val_predict and auc, the auc maybe nan.

        y_val_predict = self._model.predict(x_val)
        auc = auc_metric(y_val, y_val_predict)
        log("predict y_val shape = {}, y_val={}, y_val_predict shape = {}, y_val_predict={}, auc={}".format(y_val.shape, y_val, y_val_predict.shape, y_val_predict, auc))
        need_predict = False
                ...
                if self._test_x is None:
                    if self._model_num == 0:
                        if FEET_MODE == "LIBROSA":
                            self._test_x = self.m_data_manager.lr_preprocess(test_x)
                        else:
                            self._test_x = lr_preprocess_update(test_x)


                    elif self._round_num == 0:
                        self._test_x = self.m_data_manager.nn_preprocess(test_x,
                                                                         n_mfcc=96,
        ...
        if self.predict_loop_num >= PREDICT_BBLEDING_START_LOOP_NUM:
            return self._blending_ensemble()
        else:
            # return y_pred
            return self._k_best_predicts[0]
