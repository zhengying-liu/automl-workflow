...
def pre_trans_wav_update_bak0214(wav_list, params):
    print("pre_trans_wav len={}".format(len(wav_list)))
    if len(wav_list) == 0:
        return []

    elif len(wav_list) > NCPU * 50:
        print("note: using pool pre_trans_wav len={}".format(len(wav_list)))
        ...
        print("note: using no pool pre_trans_wav len={}".format(len(wav_list)))
...
def pre_trans_wav_update(wav_list, params):
    ...
    if len(wav_list) == 0:
        ...
        print("note: using pool pre_trans_wav len={}".format(len(wav_list)))
        # with Pool(NCPU) as pool:
        mag_arr = proc_pool.starmap(wav_to_mag, zip(wav_list, repeat(params)))
        # # import gc
        # # gc.collect()
        # # mag_arr = pool.starmap(wav_to_mag, zip(np.asfortranarray(wav_list), repeat(params)))
        # pool.close()
