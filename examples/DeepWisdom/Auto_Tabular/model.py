...
class Model(object):
  ...
  def simple_lgb(self, X, y, test_x):
    ...
    if self.train_loop_num == 1:
      if X.shape[1] > 500:
        #self.sample_cols = random.sample(list(set(X.columns)), X.shape[1]*0.5)
      ...
      #self.hyperparams['learning_rate'] -= self.train_loop_num * 0.002
      #self.hyperparams['learning_rate'] = max(0.08, self.hyperparams['learning_rate'])
      #print(self.hyperparams['learning_rate'])
  ...
  def to_numpy_train(self, dataset):
    ...
    if self.train_loop_num == 1:
      ...
      # label_distri = y.mean(axis=0)
      # ratio = np.max(label_distri)/ np.min(label_distri)
      # print(ratio)
