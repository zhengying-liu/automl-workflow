        ...
        for batch_idx in range(self.batch_size):
            if not self.fill_last_batch and self.i >= self.num_samples:
                ...
                try:
                    ...
                    # if not self.fill_last_batch:
                        # print(type(example), example.dtype, label.dtype)
                    if self.i == 0:
                        # pdb.set_trace()
                        self.example_size = example.shape
                        self.label_size = label.shape
                        # self.example_cpy = np.copy(example)
                        # self.label_cpy = np.copy(label)
...
class ExternalSourcePipeline(Pipeline):
    def __init__(self, session, dataset, batch_size, num_threads, is_random_flip=True, num_samples=1000000, device_id=0, preprocess=None, fill_last_batch=True):
        ...
        # self.frcm = ops.FastResizeCropMirror(resize_x=preprocess['width'], resize_y=preprocess['height'])
        if self.preprocess is not None:
            crop = (preprocess['width'], preprocess['height'])
            # print(crop)
            # self.res = ops.RandomResizedCrop(size=crop) 
        ...
        # samples = 1000000
        # num_samples = min(self.num_samples, samples)
    ...
    def define_graph(self):
        ...
        # if self.is_random_flip: 
            # print(output.type())
            # output = self.flip(output, horizontal=rng, vertical=rng2)
        # if self.preprocess is not None:
            # print(output.type())
...
class DALILoader(object):
    ...
    def __init__(self, session, dataset, num_samples, batch_size, steps=None, num_threads=0, fill_last_batch=True, is_random_flip=True, preprocess=None):
        ...
        pipe.build()
        # print(inspect.getargspec(DALIClassificationIterator.__init__))
        # pdb.set_trace()
