...
# def get_tf_resize(height=None, width=None, times=1, min_value=0.0, max_value=1.0):
...
def get_tf_resize(height, width, times=1, min_value=0.0, max_value=1.0):
    def preprocessor(tensor):
        ...
        if times != in_times or times > 1:
            # print('before reshpe',tensor.shape)
            ...
            # print('after reisze', tensor.shape)
...
def get_tf_to_tensor(is_random_flip=True):
    def preprocessor(tensor):
        ...
        if dims == 3:
            ...
            # time, height, width, channels -> channels, time, height, width
