...
class LogicModel(Model):
    def __init__(self, metadata, session=None, model_config=None):
        ...
        # model_name = "af_model_final.pkl"
        #
        # p = Path(__file__).parents[3] / model_name
        # model_fn = str(p)

        perf_fn = str(Path(__file__).parents[3] / "perf_matrix.csv")
        feat_fn = str(Path(__file__).parents[3] / "meta_features.csv")

        af = AFCsvFacade(perf_fn=perf_fn, feat_fn=feat_fn)

        config = {'StandardScaler': False, 'fgroup_all': True, 'imputer_strategy': 'mean', 'pca': False, 'selector': 'PairwiseClassifier',
