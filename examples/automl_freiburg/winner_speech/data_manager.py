...
class DataManager:
    ...
    def _init_each_class_index(self):
        ...
        self._max_class_num, self._min_class_num = int(np.max(each_class_count)), int(
            np.min(each_class_count)
        )
        log(
            'Raw train data: train_num(without val) {}; '.format(len(self._train_y)) +
            'class_num {} ; max_class_num {}; min_class_num {}; '.
            format(self._num_classes, self._max_class_num, self._min_class_num)
        )
    ...
    def _init_even_class_index(self):
        ...
        for i in range(self._num_classes):
            ...
            if class_cnt < sample_per_class:
                ...
                tmp += random.sample(self._each_class_index[i], sample_per_class - len(tmp))
            else:
    ...
    def _get_samples_from_each_class(self, sample_num):
        ...
        for i in range(self._num_classes):
            ...
            if class_cnt < per_class_num:
                ...
                tmp += random.sample(self._each_class_index[i], per_class_num - len(tmp))
            else:
        ...
        if model_num == 0:
            ...
            if len(train_samples) > 0:
                ...
                if self._lr_train_x is None:
                    ...
                    self._lr_train_x = np.concatenate((self._lr_train_x, train_x), axis=0)
                    self._lr_train_y = np.concatenate((self._lr_train_y, train_y),
                                                      axis=0).astype(np.int)
            if self._lr_val_x is None:
                self._lr_val_x = self.lr_preprocess(self._val_x)
                self._lr_val_y = self._val_y
            return np.asarray(self._lr_train_x), np.asarray(self._lr_train_y), np.asarray(
                self._lr_val_x
            ), np.asarray(self._lr_val_y)
            ...
            if len(train_samples) > 0:
                ...
                if self._pre_train_x is None:
                    ...
                    self._pre_train_y = np.concatenate(
                        (self._pre_train_y, np.array(train_y)), axis=0
                    ).astype(np.int)
        else:
