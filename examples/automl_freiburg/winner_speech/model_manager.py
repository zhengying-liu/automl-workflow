...
class ModelManager(Classifier):
    ...
    def _pre_select_model(self, train_loop_num):
        ...
        if self._not_rise_num == self._patience \
            ...
            self._model_idx += 1
            if self._model_idx == len(self._model_sequences) and LR_MODEL in self._model_sequences:
                # TODO be careful!
                self._model_idx = 1
                self._round_num += 1
            ...
            self._use_new_train = self._model_num in [
                0, 1, (2 * (len(self._model_sequences) - 1)) + 1,
                (3 * (len(self._model_sequences) - 1)) + 1,
                (4 * (len(self._model_sequences) - 1)) + 1
            ]
            self._is_reset_model = (
                self._round_num > 1 and
                self._model_num == self._round_num * (len(self._model_sequences) - 1) + 1
            )
    ...
    def _get_each_model_top_k_predicts(self):
        ...
        for k, v in self._each_model_best_auc.items():
            ...
            temp = [
                (auc, k_predicts[i]) for i, auc in enumerate(v)
        ...
        predicts = sorted(predicts, key=lambda x: x[0], reverse=True)[:self._each_model_keep_num]
    ...
    def _blending_ensemble(self):
        ...
        log(
            "model_num: {} Select k best {} predicts which have auc {}, ".
            format(self._model_num, self._keep_num, self._k_best_auc) +
            "each model {} best which have auc {}, ".
            format(self._each_model_keep_num, each_model_k_aucs) +
    ...
    def predict(self, test_x, is_final_test_x=False):
        ...
        if auc > self._cur_model_max_auc:
            log(
                'cur_max_auc {}; cur_auc {}; {} auc rise for {} times'.format(
                    self._cur_model_max_auc, auc, self._model_name, self._cur_model_run_loop
                )
            )
        ...
        if max(
            self._k_best_auc[-1], self._each_model_best_auc[LR_MODEL][-1] - 0.1
        ) >= auc and not need_predict:
            log('not predict')
        else:
            log('new predict')
            if is_final_test_x:
