...
from data_process import (
    extract_melspectrogram_parallel, extract_mfcc_parallel, get_max_length, ohe2cat, pad_seq
)
from models.my_classifier import Classifier
from tensorflow.python.keras.layers import (
    Activation, BatchNormalization, Concatenate, Conv2D, Dense, Dropout, Flatten,
    GlobalAveragePooling2D, GlobalMaxPooling2D, Input, MaxPooling2D
)
...
class CnnFeatures(Classifier):
    ...
    def init_model(self, input_shape, num_classes, **kwargs):
        ...
        for i in range(5):
            if i == 0:
                conv_l = Conv2D(
                    64, 3, input_shape=input_shape, padding='same', data_format='channels_last'
                )(inputs)
            else:
