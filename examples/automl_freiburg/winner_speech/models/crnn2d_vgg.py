...
class Crnn2dVggModel(Classifier):
    ...
    def init_model(self, input_shape, num_classes, **kwargs):
        ...
        # Conv block 1
        x = Convolution2D(
            filters=filters_size[0], kernel_size=kernel_size, padding='same', name='conv1_1'
        )(x)
        x = ELU()(x)
        x = BatchNormalization(axis=channel_axis, name='bn1_1')(x)
        x = Convolution2D(
            filters=filters_size[0], kernel_size=kernel_size, padding='same', name='conv1_2'
        )(x)
        x = ELU()(x)
        ...
        for layer in range(1, layers):
            ...
            x = MaxPooling2D(
                pool_size=pool_size[layer], strides=pool_size[layer], name='pool{layer + 1}'
            )(x)
