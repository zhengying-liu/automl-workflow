...
from data_process import (
    extract_bandwidth_parallel, extract_chroma_stft_parallel, extract_melspectrogram_parallel,
    extract_mfcc_parallel, extract_spectral_centroid_parallel, extract_spectral_contrast_parallel,
    extract_spectral_flatness_parallel, extract_spectral_rolloff_parallel,
    extract_zero_crossing_rate_parallel, ohe2cat
)
...
class SvmModel(Classifier):
    ...
    def init_model(self, kernel, max_iter=800, C=1.0, **kwargs):
    ...
    def preprocess_data(self, x):
        ...
        for i in range(len(x_mfcc)):
            ...
            x_feas.append(
                np.concatenate(
                    [mfcc, mel, chroma_stft, contrast, flatness, cent, bw, rolloff, zcr], axis=-1
                )
            )
