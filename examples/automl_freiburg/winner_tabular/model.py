...
class Model(object):
    ...
    """
        ...
        self.classifier = tf.estimator.Estimator(
            model_fn=model_fn, config=tf.estimator.RunConfig(session_config=session_config)
        )
    ...
    ****************************************************************************
    ...
    """
        ...
        if steps_to_train <= 0:
            logger.info("Not enough time remaining for training + test. " + "Skipping training...")
    ...
    """
        ...
        remaining_time_budget = min(remaining_time_budget - 60, remaining_time_budget * 0.95)
...
def crop_time_axis(tensor_4d, num_frames, begin_index=None):
  ...
  """
    ...
    sliced_tensor = tf.slice(padded_tensor, begin=begin_index, size=[num_frames, -1, -1, -1])
...
def resize_space_axes(tensor_4d, new_row_count, new_col_count):
  ...
  """
    resized_images = tf.image.resize_images(tensor_4d, size=(new_row_count, new_col_count))
